...
import org.jvmtorch.impl.MultiClassCrossEntropyLossImpl;
...
public class ML4JMultiClassCrossEntropyLossImpl extends MultiClassCrossEntropyLossImpl {
    ...
    public ML4JMultiClassCrossEntropyLossImpl(Torch torch, TensorDataConverter<ML4JTensorOperations> tensorDataConverter, TensorConverter<ML4JTensor> tensorConverter) {
        this.torch = torch;
        this.costFunction = new MultiClassCrossEntropyCostFunction();
    ...
    @Override
    public Tensor forward(MultiClassCrossEntropyLoss self,
                                                Tensor input,
                                                Tensor target) {
		...
		float cost  = costFunction.getAverageCost(targetMatrix.transpose(), inputMatrix.transpose());
    	...
    	DirectedComponentGradient<NeuronsActivation> gradient = costFunctionGradient.backPropagateThroughFinalActivationFunction(ActivationFunctionType.getBaseType(ActivationFunctionBaseType.SOFTMAX));
    	
    	NeuronsActivation output = gradient.getOutput();
    	
    	ML4JTensor out2 =  new ML4JTensor(torch, ml4jTensor.getDirectedComponentsContext(), tensorDataConverter, output, false);    

        Tensor outputTensor = input.performUnaryMappingOperation(
                new TensorOperationImpl<TensorData, Size>(torch, "LossOutput", l -> 
