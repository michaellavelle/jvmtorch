...
public class RowVectorAddition<T extends TensorOperations<T>> extends DifferentiableTensorTensorFunctionBase<T> {
	
	public RowVectorAddition(Torch torch) {
		super(torch);
	}
	...
	public <S extends TensorOperations<S>> Pair<UnaryOperator<S>, UnaryOperator<S>> backPropFunctions(Pair<S, S> variables) {
		return new ImmutablePair<>(g -> g, g -> g.columnSums());
	}
